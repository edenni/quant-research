...
class Alphas(object):
    ...
    def calc_alpha(cls, path, func, data):
        try:
            ...
            t2 = time.time()
            print(
                f"Factory {os.path.splitext(os.path.basename(path))[0]} time {t2-t1}"
            )
    ...
    def generate_alphas(cls, year, list_assets, benchmark):
        ...
        for m in methods:
            ...
            try:
                pool.apply_async(
                    cls.calc_alpha, (f"{path}/{m}.csv", factor, stock)
                )
            except Exception as e:
                traceback.print_exc()
...
class Alphas101(Alphas):
    ...
    def alpha021(self):
        ...
        cond_2 = sma(self.close, 2) < sma(self.close, 8) - stddev(
            self.close, 8
        )
        cond_3 = sma(self.volume, 20) / self.volume < 1

        return (cond_1 | ((~cond_1) & (~cond_2) & (~cond_3))).astype("int") * (
            -2
        ) + 1
    ...
    def alpha031(self):
        ...
        p1 = rank(
            rank(
                rank(
                    decay_linear((-1 * rank(rank(delta(self.close, 10)))), 10)
                )
            )
        )
    ...
    def alpha068(self):
        ...
        return (
            ...
            < rank(
                delta(
                    ((self.close * 0.518371) + (self.low * (1 - 0.518371))), 2
                )
            )
            * 14
        ) * -1
    ...
    def alpha071(self):
        ...
        p1 = ts_rank(
            decay_linear(
                correlation(ts_rank(self.close, 3), ts_rank(adv180, 12), 18), 4
            ),
            16,
        )
        p2 = ts_rank(
    ...
    def alpha072(self):
        ...
        return rank(
            decay_linear(
                correlation(((self.high + self.low) / 2), adv40, 9), 10
            )
        ) / rank(
            decay_linear(
                correlation(
    ...
    def alpha077(self):
        ...
        p2 = rank(
            decay_linear(
                correlation(((self.high + self.low) / 2), adv40, 3), 6
            )
        )
        return min(p1, p2)
    ...
    def alpha088(self):
        ...
        p2 = ts_rank(
            decay_linear(
                correlation(ts_rank(self.close, 8), ts_rank(adv60, 21), 8), 7
            ),
            3,
        )
        return min(p1, p2)
    ...
    def alpha092(self):
        ...
        p2 = ts_rank(
            decay_linear(correlation(rank(self.low), rank(adv30), 8), 7), 7
        )
        return min(p1, p2)
    ...
    def alpha096(self):
        ...
        p1 = ts_rank(
            decay_linear(
                correlation(rank(self.vwap), rank(self.volume), 4), 4
            ),
            8,
        )
        p2 = ts_rank(
    ...
    def alpha098(self):
        ...
        adv15 = sma(self.volume, 15)
        return rank(
            decay_linear(correlation(self.vwap, sma(adv5, 26), 5), 7)
        ) - rank(
            decay_linear(
                ts_rank(
                    ts_argmin(
